...
def sync_watchlist_with_mapping(watchlist: pd.DataFrame, id_map: dict) -> pd.DataFrame:
    """
    Ensure every ticker in id_map exists in watchlist.
    Does NOT delete watchlist rows that are not in the mapping (safer).
    """
    wl = watchlist.copy()
    if wl.empty:
...
# UI styling
# -----------------------------
st.set_page_config(page_title="🍄 Super Dividend Dashboard", page_icon="🍄", layout="wide")
...
.muted { opacity: 0.72; font-size: 0.92rem; line-height: 1.5; }
...
# Always keep watchlist in sync with mapping on load
...
# Header
# -----------------------------
st.markdown("## 🍄 Super Dividend Dashboard")
st.markdown(
    '<div class="muted"><b>Raw dividends</b> are pulled from <b>dividends.ph</b> (last 10 max). '
    'Price + stock snapshot come from <b>PSE Edge</b> using a hidden <code>cmpy_id</code>. '
    'Annual totals are manual (your input) or auto-derived from raw dividends for last 4 years.</div>',
    unsafe_allow_html=True,
)
...
with st.expander("✨ Add Ticker Wizard (recommended)"):
    st.write("Add a ticker by entering **ticker** and **cmpy_id** (from PSE Edge URL). cmpy_id stays hidden in watchlist.")
    colA, colB, colC = st.columns([1, 1, 1])
    with colA:
        new_ticker = st.text_input("Ticker (e.g. MER)", value="", key="wiz_ticker").upper().strip()
    with colB:
        new_cmpy = st.text_input("cmpy_id (number)", value="", key="wiz_cmpy")
    with colC:
        add_clicked = st.button("➕ Add", type="secondary", use_container_width=True)

    if add_clicked:
        if not new_ticker or not new_cmpy.strip().isdigit():
            st.error("Please enter a ticker and a numeric cmpy_id.")
        else:
            st.session_state.cmpy_id_map[new_ticker] = int(new_cmpy.strip())
            st.session_state.watchlist = sync_watchlist_with_mapping(st.session_state.watchlist, st.session_state.cmpy_id_map)
            st.success(f"Added/updated {new_ticker}.")

with st.expander("🔧 Advanced: view/edit cmpy_id mapping"):
    map_df = pd.DataFrame([{"Ticker": t, "cmpy_id": cid} for t, cid in sorted(st.session_state.cmpy_id_map.items())])
    edited_map = st.data_editor(map_df, num_rows="dynamic", use_container_width=True, hide_index=True)
    if st.button("Save cmpy_id changes", type="secondary"):
        new_map = {}
        for r in edited_map.fillna("").to_dict(orient="records"):
            t = normalize_ticker(r.get("Ticker", ""))
            c = str(r.get("cmpy_id", "")).strip()
            if t and c.isdigit():
                new_map[t] = int(c)
        if new_map:
            st.session_state.cmpy_id_map.update(new_map)
            # ✅ Sync watchlist now too
            st.session_state.watchlist = sync_watchlist_with_mapping(st.session_state.watchlist, st.session_state.cmpy_id_map)
...
st.markdown("### ⭐ Watchlist (cmpy_id hidden)")
...
if sort_clicked:
    wl = wl.sort_values("Ticker").reset_index(drop=True)
if clear_divs:
    wl["Dividends (annual totals, oldest→newest)"] = ""
if clear_notes:
    wl["Notes"] = ""

edited_wl = st.data_editor(wl, num_rows="dynamic", use_container_width=True, hide_index=True)
st.session_state.watchlist = edited_wl
...
if run_clicked:
    # ✅ Sync watchlist with mapping at the moment we run valuation
    st.session_state.watchlist = sync_watchlist_with_mapping(st.session_state.watchlist, st.session_state.cmpy_id_map)

    rows = st.session_state.watchlist.fillna("").to_dict(orient="records")
    with st.spinner("🍄 Powering up… fetching stock data + dividends + computing values"):
...
if isinstance(df, pd.DataFrame) and not df.empty:
    ...
    st.download_button("⬇️ Download results (CSV)", data=csv, file_name="pse_valuation_results.csv", mime="text/csv")
    ...
    selected = st.selectbox("Choose your character (ticker)", tickers_available, key="selected_ticker")
    ...
    with cA:
        ...
        if snap_df.empty:
            st.info("No snapshot fields parsed for this ticker (PSE Edge format may have changed).")
        else:
        ...
        if isinstance(raw_df, pd.DataFrame) and not raw_df.empty:
            raw10 = raw_df.copy().sort_values("payment_date", ascending=False).head(10)
            raw10_for_plot = raw10.sort_values("payment_date")
            ...
            with left_raw:
                ...
                axr.plot(raw10_for_plot["payment_date"], raw10_for_plot["rate"], marker="o")
                axr.set_xlabel("Payment date (MM-YY)")
                axr.set_ylabel("Dividend rate (per payment)")
                axr.set_title(f"{selected} raw dividends (last 10)")

                axr.xaxis.set_major_formatter(mdates.DateFormatter("%m-%y"))
                axr.xaxis.set_major_locator(mdates.MonthLocator(interval=1))

                # ✅ smaller tick font so it stays readable
            ...
            with right_raw:
                display_df = raw10.copy()
                display_df["payment_date"] = display_df["payment_date"].dt.strftime("%m-%y")
                display_df["ex_date"] = display_df["ex_date"].dt.strftime("%m-%y")
                display_df = display_df.rename(columns={
                    "payment_date": "Payment (MM-YY)",
                    "ex_date": "Ex (MM-YY)",
                    "rate": "Rate",
                    "year": "Year",
                })
    ...
    st.info("Press **🪙 Run valuation** to generate results, then you’ll see 🍄 Stock & Dividend Data.")
